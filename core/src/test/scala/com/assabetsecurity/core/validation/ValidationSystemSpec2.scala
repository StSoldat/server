 ...
 */
class ValidationSystemSpec2  extends WordSpec with ShouldMatchers with BeforeAndAfter with BeforeAndAfterAll with Logging {
  ...
  def testSite2 = Site( id = new SiteId(UUID.fromString("34009e87-66c6-4360-ad77-64cf34c730b9")),
    ...
    siteConfigs = Map(
      ...
      "default" -> SiteConfig(name = "default",
        postValidationSuccess = Some(true),
        postValidationFailure = Some(true),
        validationClass =
  ...
  "system" should {
    ...
    "validate block plain text message" in {
      val m = SiteMessageContent(
        ...
        serverIp = None,
        ip = None,
        message = "Test fs@com dot.com balue:test Message aaa aab ffck  a@a.com")  //aab is in list
      ...
      res.asInstanceOf[ValidationResults].results.find(v=>v.isInstanceOf[TextValidationResultFailure]).size should equal(0)
      Thread.sleep(2000)
    }
